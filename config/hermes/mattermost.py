...
class MattermostAdapter(BasePlatformAdapter):
    ...
    async def _is_bot_thread(self, post_id: str) -> bool:
        """Check if the thread contains any post by the bot."""
        if not post_id:
            return False
        thread_data = await self._api_get(f"posts/{post_id}/thread")
        if not thread_data or "posts" not in thread_data:
            return False
        for post in thread_data["posts"].values():
    ...
    async def send(
        ...
        # Robustly resolve thread ID from reply_to or metadata
        reply_id = (
            reply_to
            or (metadata.get("thread_id") if metadata else None)
            or (metadata.get("message_id") if metadata else None)
            or (metadata.get("post_id") if metadata else None)
        )
    ...
    async def _handle_ws_event(self, event: Dict[str, Any]) -> None:
        ...
        if channel_type_raw != "D":
            ...
            # If the post is inside a thread, check if the bot is already a participant.
            thread_id = post.get("root_id") or None
            is_bot_thread = False
            if thread_id:
                is_bot_thread = await self._is_bot_thread(thread_id)

            if require_mention and not is_free_channel and not has_mention and not is_bot_thread:
                logger.debug(
