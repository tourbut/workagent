...
def get_memory_dir() -> Path:
    ...
    try:
        ...
        if user_name:
            ...
            safe_user_name = "".join(c for c in user_name if c.isalnum() or c in ("-", "_")).lower()
            if safe_user_name:
                path = get_hermes_home() / "memories" / "users" / safe_user_name
                path.mkdir(parents=True, exist_ok=True)
                return path
    except Exception as e:
        logger.error(f"Error resolving user isolated memories directory: {e}")
...
class MemoryStore:
    ...
    def __init__(self, memory_char_limit: int = 2200, user_char_limit: int = 1375):
        ...
        # Cache memory directory during active session to survive background context loss
